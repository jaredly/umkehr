...
const inverts = <T>(change: Patch<T>[]) =>
	change.toReversed().map((op) => ops.invert(op));
...
export function update<T>(
	...
	const history = (
		state.back > 0 ? state.history.slice(0, -state.back) : state.history
	).concat([{ ops: update, ts: Date.now() }]);
	let value = state.value;
	update.forEach((op) => {
		value = ops.apply(value, op, equal);
	});
