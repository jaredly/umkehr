...
export function createPatchBuilder<T, Extra, Tag extends string = "type">(
	tag: Tag,
	extra: Extra,
) {
	return createPatchDispatcher<T, Extra, Tag, DraftPatch<T, Tag, Extra>>(
		(x) => x,
		extra,
		tag,
	);
	...
	function makeProxy(path: Array<PathSegment>): any {
		...
		const handler: ProxyHandler<any> = {
			get(_target, prop, _receiver) {
				...
				if (!proxyCache[k])
					proxyCache[k] = makeProxy([...path, { type: "key", key }]);
				return proxyCache[k];
			},
		};
