...
function _getCloned(root: any, at: PathSegment[]) {
	...
	for (let i = 0; i < at.length; i++) {
		...
		parent = base;
		parentKey = key;
		base[key.key] = Array.isArray(base[key.key])
			? base[key.key].slice()
			: { ...base[key.key] };
		base = base[key.key];
	}
...
export function _replace(
	...
	if (key.type !== "key")
		throw new Error(`weird final key type while replacing: ${key.type}`);
	if (Array.isArray(base)) {
		if (typeof key.key !== "number") {
			throw new Error(`invalid key for array: ${key.key}`);
		}
...
export function _add(base: any, at: PathSegment[], value: any) {
	...
	const key = at[at.length - 1];
	if (key.type !== "key")
		throw new Error(`weird final key type while adding ${key.type}`);
	if (Array.isArray(base)) {
		if (typeof key.key !== "number") {
			throw new Error(`invalid key for array: ${key.key}`);
		}
		...
		throw new Error(
			`key "${key.key}" already exists, cannot add, must replace`,
		);
	} else {
		base[key.key] = value;
	}
...
export function _remove(
	...
	const key = at[at.length - 1];
	if (key.type !== "key")
		throw new Error(`weird final key type while removing ${key.type}`);
