...
import type {BlockOrderTs, BlockStyle, CachedState, CharParentTs, HLC, Op, State} from '../block-crdt/types.js';
...
export type BlockEditorReplica = {
    id: BlockEditorId;
    actor: string;
...
type Replica = BlockEditorReplica;
type EditorId = BlockEditorId;

type BlockEditorUndoState = {
    canUndo: boolean;
    canRedo: boolean;
    undoReason?: string;
    redoReason?: string;
...
export type BlockEditorKeyPerfSampleInput = {
    editorId: string;
    label: string;
    ms: number;
    blockId?: string;
...
const makeCommandContext = (replica: Replica) => ({
    actor: replica.actor,
        ...
        <article className={replica.online ? 'editorPanel' : 'editorPanel offline'}>
            <header className="editorHeader">
                <div>
                    ...
                    <span>
                        {replica.online === false ? 'offline' : 'online'} · queued{' '}
