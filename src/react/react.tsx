...
type QueuedChanges<T, Tag extends string = "type"> = DraftPatch<
	T,
	Tag,
	Extra
>[];

type CH<T, An, Tag extends string = "type"> = ContextBase<
	History<T, An>,
	T,
	Tag
> &
...
const addPathListener = (
	root: PathListenerNode,
	path: Path,
...
const removePathListener = (
	root: PathListenerNode,
	path: Path,
	listener: PathListener,
) => {
	const stack: Array<{ node: PathListenerNode; key?: string }> = [
		{ node: root },
	];
	let node = root;
	for (const seg of path) {
...
const makeDispatch = <T, Tag extends string = "type">(
	...
	const go = (
		v: MaybeNested<DraftPatch<T, Tag, Extra>>,
		when?: ApplyTiming,
	) => {
		if (when === "preview") {
