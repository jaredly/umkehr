...
- Follow-up complete: replaced the earlier mouseup-driven selection timing with actual DOM selection timing. A click inside editable content now starts a pending timer on mousedown and records `DOM selection` when the document `selectionchange` event fires and `readSelectionFromDom(root)` can read the updated selection.
- Added a one-second stale cleanup for pending DOM-selection timers so clicks that never produce a readable editor selection do not leak pending measurements.
...
- Selection timing note: DOM selection timing measures mousedown-to-`selectionchange`, not the later React selection capture/update path. Keyboard-driven selection capture via `keyup` remains unmeasured so typing/navigation samples are not conflated with click selection timings.
