...
- Follow-up bug: creating an annotation body could throw `block order path ... references a missing block` because cache reorganization still derived virtual parents from block-only inputs, so mark-based parents were unavailable after the block op applied. Fixed cache organization and block-parent derivation to accept mark data when rebuilding caches, and added a regression that creates an annotation body under a mark virtual parent.
