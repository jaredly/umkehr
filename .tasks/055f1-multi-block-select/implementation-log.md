...
- Added a table-cell drag affordance cursor. The focused selected cell gets a transient `cellDragReady` state, and `grab`/`grabbing` cursors appear only while hovering the border where a selected cell drag can actually start.
