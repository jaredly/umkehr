...
- Fixed server architecture render loop: split `TopBarContext` into separate state and dispatch contexts so components registering top-bar controls do not subscribe to top-bar control updates. Verified server mode in headless Playwright with no maximum-update-depth console/page errors, then re-ran typecheck and build successfully.
