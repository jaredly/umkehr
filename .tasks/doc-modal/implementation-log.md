...
- Merged the visible app picker entries for Todos and Todos migration. The migration fixture remains as a hidden compatibility registration for direct URLs/tests because it has a distinct schema/runtime, but the app selector now only shows one Todos entry.
