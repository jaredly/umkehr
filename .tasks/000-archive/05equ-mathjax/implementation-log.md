...
- Error rendering fix: MathJax can return serialized `merror`/`data-mjx-error` HTML without throwing. The renderer now treats that output as a failed render and caches raw source text instead of showing MathJax's visual error block. Verification: renderer regression test, focused math/document tests, typecheck, and build pass.
- Selection rendering fix: math source styling is now paint-only while selected. Removed layout-affecting padding from `.markMath` and block/min-width/margin/text alignment from `.markMathDisplay`, because drag selection splits math source into moving chunks and repeated per-chunk spacing made characters jump. Verification: focused math tests, typecheck, and build pass.
