...
- Expanded the table-cell drag affordance to normal text selections inside a cell. The edge cursor now matches the actual drag behavior whenever the cell is the active selected cell, not only when the selection is already `table-cells`.
- Extended selected-cell dragging for text ranges that span multiple cells. Starting a cell drag from the focused selected cell now converts the touched text-range endpoints into a table-cell rectangle and moves all cells in that rectangle together.
